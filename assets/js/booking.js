...
document.getElementById('display-course-name').textContent = urlParams.get('courseName') || '課程名稱';
document.getElementById('teacher-name').textContent = urlParams.get('tutorName') || '老師名稱';
            ...
            for (let weekOffset = 0; weekOffset < TOTAL_WEEKS; weekOffset++) {
                ...
                const isBooked = bookings.some(b => {
                    let bDateStr = Array.isArray(b.date)
                        ...
                        : b.date;
                    return bDateStr === dateString && b.hour === s.hour && b.slotLocked === true;
                });
...
function renderAllWeeks() {
    ...
    for (let w = 0; w < TOTAL_WEEKS; w++) {
        ...
        weekDates.forEach((date, i) => {
            ...
            dayHeader.className = 'calendar-day-header' + (daySlots.length > 0 ? ' has-slots' : '');
            dayHeader.innerHTML = `
            ...
            if (daySlots.length === 0) {
                ...
                daySlots.sort((a, b) => a.hour - b.hour).forEach(slot => {
...
function updateTrackPosition() {
    document.getElementById('calendar-track').style.transform = `translateX(-${currentWeek * 100}%)`;
}
...
function updateNavButtons() {
    ...
    document.getElementById('next-week').disabled = currentWeek === TOTAL_WEEKS - 1;
